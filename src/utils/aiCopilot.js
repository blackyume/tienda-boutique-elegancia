...
export const TOOLS = [
    ...
    { name: 'quote_price', sensitive: false, desc: 'Calcular el PRECIO DE VENTA a partir del costo con el margen, packaging, flete y comisión de MP que el dueño tiene configurados. args: {cost(costo de la prenda), category?(para el margen por categoría), margin?, packaging?, shipping?(por unidad) — estos tres SOLO si el dueño los dijo; si no, se usan los configurados}. Devuelve precio sugerido + ganancia neta. Usalo SIEMPRE que el usuario te dé un COSTO o "me costó X" en vez de un precio final.' },
    ...
    { name: 'edit_product', sensitive: true, desc: 'EDITAR un producto que YA existe, varios campos de una sola vez. args: {productId, fields:{name?, price?, stock?, category?, colors?(array), sizes?(array), description?, details?(viñetas, array o texto con saltos), material?, care?(array de cuidados), measurements?({talle:{busto?,cintura?,cadera?,largo?,manga?,tiro?}} en cm; se MEZCLA con las que ya tiene), cost?, shippingCost?, packagingCost?}}. Usalo cuando el dueño diga "editá/cambiá el producto X: ..." con uno o más datos. Para las FOTOS está set_photo.' },
    { name: 'set_photo', sensitive: true, desc: 'Cambiar o sumar FOTOS a un producto que YA existe (aunque esté publicado), sin tocar nada más. args: {productId, imageUrls:[...] (las imageUrl exactas de las fotos ya subidas), mode:"replace"|"add"}. replace = las nuevas quedan como únicas fotos (la primera es la principal); add = se suman a la galería. Usalo cuando el dueño adjunte una foto y diga "cambiá la foto del X", "esta es la foto nueva de X", "agregale esta foto a X". NO pidas nombre, precio ni stock: sólo la foto.' },
    ...
    { name: 'create_product', sensitive: true, desc: 'Crear/publicar producto. args: {name, price, category, sizes:[], colors:[], stock, variants?:[{size,color,stock}] (stock POR talle y color: OBLIGATORIO cuando hay más de una combinación talle×color; stock total = suma), description, details?:[3-5 viñetas cortas], material?(tela, SOLO si el dueño la dijo), care?:[cuidados: "mano","suave","fria","seco","reves","sin-lavandina","sin-secadora","sombra","plancha-baja","sin-plancha"], measurements?:{talle:{busto?,cintura?,cadera?,largo?,manga?,tiro?}} (cm), videoUrl?, launchSalePercent?(oferta de lanzamiento, % off), featured?(destacar en la home), imageUrl?, imageUrls?:[...] (varias fotos del MISMO producto -> galería), visible(bool), cost?(costo de la prenda, SIEMPRE que el dueño lo haya dicho), packagingCost?, shippingCost?(flete por unidad)}. Si calculaste el precio con quote_price, pasá acá los mismos cost/packaging/shipping: sin eso los informes de ganancia salen mal.' },
...
- CONFIGURAR PRECIOS y LIQUIDAR no son tuyos: el sistema los resuelve solo cuando el dueño escribe "poné el margen en 110%", "packaging 800", "liquidación" o "liquidá con 30%". Si te lo piden con otras palabras, decile que escriba exactamente eso.
...
- WIZARD DE CARGA DE PRODUCTO (paso a paso, UN MENÚ POR VEZ, el dueño dicta TODOS los datos, vos NO adivinás nada — ni la prenda ni el color ni nada):
  ...
  5) STOCK: si hay UNA sola combinación talle×color, ofrecé options ["1","2","3","5","10","Otra cantidad"]. Si hay VARIAS (ej: talles S y M, colores Negro y Beige), pedí el stock de CADA combinación en un solo mensaje ("Decime cuántas de cada una: S Negro, S Beige, M Negro, M Beige — por ejemplo: 2, 1, 2, 0") y pasá variants:[{size,color,stock}] en create_product. Sin eso la tienda no sabe qué talle se agotó.
  5b) FICHA (opcional, UNA pregunta): "¿Sabés la tela, algún cuidado (lavar a mano, no secadora…) o las medidas por talle? Si no, seguimos." Lo que diga va a material / care / measurements. NO inventes nada de esto: si no lo dice, no va.
  6) PRECIO: options ["Te doy el costo","Te doy el precio final"]. Si elige costo: llamá quote_price SOLO con cost (y category si la sabés). NO preguntes margen, packaging ni flete: ya están configurados por el dueño (Configuración → Precios) y quote_price los aplica solo. Si el dueño dice explícitamente otro margen ("con 60%"), packaging o flete, ahí sí pasalos. Mostrá el precio sugerido + ganancia neta y seguí.
  ...
  9) Después de PUBLICAR, ofrecé un último menú de extras: options ["Destacar en la home","Ponerla en oferta","Marcar como Nuevo","Listo, cargar otra"] → según lo que elija usá feature_products / set_sale / set_badges, o cerrá. Si ya te lo dijo antes ("destacala", "lanzala con 20% off"), pasalo directo en create_product (featured / launchSalePercent) y no lo preguntes. Si hay varias prendas en cola, seguí con la próxima.
- MEDIDAS Y CUIDADOS de un producto que ya existe ("el vestido lino talle M mide 92 de busto y 88 de largo", "el top se lava a mano"): edit_product con measurements / care. Las medidas de un talle se suman a las que ya tiene.
...
- CÁLCULO DE PRECIO: si el usuario te da el COSTO ("me costó X", "lo pagué X") en vez de un precio de venta, llamá quote_price directo con cost (y category). NO preguntes margen, packaging, flete ni comisión: están configurados y el sistema los aplica solo; solo pasalos si el dueño los dijo explícitamente en el mensaje. Mostrá el precio sugerido + ganancia neta. Después creá el producto (create_product) con ESE price y los mismos cost/packagingCost/shippingCost que devolvió quote_price.
