...
export const StoreProvider = ({ children }) => {
  ...
  // Shipping Rates by Province (preloaded with estimates)
  ...
  useEffect(() => {
    ...
    const unsubAiHistory = onSnapshot(collection(db, "ai_history"), (snap) => {
      const data = snap.docs.map(d => ({ ...d.data(), id: d.id }));
      setAiHistory(data.sort((a, b) => b.timestamp - a.timestamp).slice(0, 50)); // Keep last 50
    });
    ...
    // Wishlist Events (for tracking)
    const unsubWishlist = onSnapshot(collection(db, "wishlist_events"), (snap) => {
      const data = snap.docs.map(d => ({ ...d.data(), id: d.id }));
      setWishlistEvents(data.sort((a, b) => b.timestamp - a.timestamp).slice(0, 500)); // Last 500
    });

    // Shipping Rates by Province (from Firestore if exists, else use defaults)
  ...
  const trackWishlistEvent = async (product, action) => {
    try {
      // Limit events to prevent free tier abuse
  ...
  const dbActions = {
    ...
    clearAiHistory: async () => {
      if (!isAdmin) return;
      try {
        const snap = await collection(db, "ai_history");
    ...
    executeScheduledPromotion: async (promo) => {
      ...
      try {
        ...
        if (promo.type === 'activate_coupon') {
          ...
          if (coupon) {
            await updateDoc(doc(db, "coupons", String(coupon.id)), { active: true });
          }
        } else if (promo.type === 'deactivate_coupon') {
          const coupon = coupons.find(c => c.code === promo.couponCode);
          if (coupon) {
            await updateDoc(doc(db, "coupons", String(coupon.id)), { active: false });
          }
  ...
  return (
    <StoreContext.Provider value={{
      ...
      visitCount, incrementVisits, paymentConfig, coupons, suppliers, aiHistory, scheduledPromotions, wishlistEvents, trackWishlistEvent,
