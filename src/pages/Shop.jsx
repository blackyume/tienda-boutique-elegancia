...
export const Shop = () => {
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-[#1C1F25] text-slate-900 dark:text-white pt-24 transition-colors">
            <SEO
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {allColors.map((c) => (
                                        <button
                                            ...
                                            title={c}
                                            className={`w-7 h-7 rounded-full border transition-all ${colors.has(c) ? 'ring-2 ring-cielo-gold ring-offset-2 dark:ring-offset-[#1C1F25] scale-110' : 'border-slate-200 dark:border-slate-700 hover:scale-105'}`}
            ...
            {/* Modal mobile */}
            {isMobileFilterOpen && (
                <div className="fixed inset-0 z-50 bg-white dark:bg-[#1C1F25] p-6 animate-slideUp overflow-y-auto">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {allColors.map((c) => (
                                    <button
                                        ...
                                        onClick={() => toggleIn(colors, c, setColors)}
                                        title={c}
                                        className={`w-8 h-8 rounded-full border ${colors.has(c) ? 'ring-2 ring-cielo-gold ring-offset-2 dark:ring-offset-[#1C1F25]' : 'border-slate-200 dark:border-slate-700'}`}
                                        style={{ background: getColorHex(c, colorHexMap) }}
                                        aria-label={c}
                                    />
                                ))}
