...
export const SettingsHealthPanel = ({ onJump }) => {
    ...
    const checks = [
        ...
        { id: 'cer',  ok: Boolean((aiConfig?.cerebrasKey || '').trim()), label: 'Cerebras (Lau)', okHint: 'Motor principal activo', badHint: 'Sin Cerebras el copiloto cae a Gemini si está', goto: 'ia' },
        { id: 'gma',  ok: Boolean((aiConfig?.adminKeys || '').trim()), label: 'Gemini · Admin (visión primaria)', okHint: 'Análisis de fotos disponible', badHint: 'Sin Gemini ni NVIDIA, Lau no puede analizar fotos', goto: 'ia' },
        { id: 'nv',   ok: Boolean((aiConfig?.nvidiaKey || '').trim()), label: 'NVIDIA NIM · Visión fallback', okHint: 'Activo como fallback de Gemini', badHint: 'Opcional. Sin esto, si Gemini falla no hay fallback de visión', goto: 'ia' },
        ...
        <div className="bg-white dark:bg-[#1a1a1a] p-6 rounded-2xl border dark:border-slate-700 shadow-sm">
            <div className="flex items-center justify-between mb-5">
                <h3 className="font-bold flex items-center gap-2 text-slate-800 dark:text-white">
                    <Activity className="w-5 h-5 text-[#E8C65E]" /> Salud de la tienda
                </h3>
