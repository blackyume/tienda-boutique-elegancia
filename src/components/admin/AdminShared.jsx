...
const SPARK_STROKE = {
    emerald: '#10b981', blue: '#3b82f6', purple: '#a855f7', orange: '#D4AF37'
};
...
export const StatCard = ({ label, value, sub, icon: Icon, theme, spark }) => {
    ...
    const themeStyles = {
        ...
        orange: "text-[#D4AF37] after:bg-[#D4AF37]", // Generic Gold/Orange
    };
    ...
    return (
        <div className="relative overflow-hidden bg-white dark:bg-[#1a1a1a] p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 group hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            {/* Elegant Accent Line */}
            ...
            <div className="flex justify-between items-start mb-4">
                ...
                <div className={`p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-400 group-hover:text-[#D4AF37] transition-colors duration-300`}>
...
export const ActionButton = ({ icon: Icon, label, onClick, color }) => {
    ...
    if (!isSpecial) {
        return (
            <button
                onClick={onClick}
                className="group relative flex flex-col items-center justify-center p-6 bg-white dark:bg-[#1a1a1a] rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden"
            >
                ...
                <div className="relative p-3 mb-3 rounded-full bg-slate-50 dark:bg-slate-800 text-slate-400 group-hover:bg-[#D4AF37] group-hover:text-white transition-all duration-300 transform group-hover:scale-110 shadow-sm">
    ...
    return (
        <button
            onClick={onClick}
            className="w-full h-full relative group overflow-hidden rounded-2xl p-6 flex flex-row sm:flex-col items-center justify-between sm:justify-center gap-4 bg-[#0a0a0a] border border-[#D4AF37]/30 shadow-lg shadow-[#D4AF37]/10 hover:shadow-[#D4AF37]/20 transition-all duration-500 hover:-translate-y-1"
        >
            {/* Animated Background */}
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-[#D4AF37]/20 via-transparent to-transparent opacity-50" />

            <div className="relative p-3 rounded-xl bg-[#D4AF37]/10 text-[#D4AF37] border border-[#D4AF37]/20 group-hover:bg-[#D4AF37] group-hover:text-black transition-all duration-500">
                <Icon className="w-6 h-6 animate-pulse-slow" />
            </div>

            <div className="relative text-left sm:text-center">
                <span className="block text-[10px] text-[#D4AF37] font-mono mb-1 opacity-80">NEURAL CORE</span>
