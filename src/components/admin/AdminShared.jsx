...
const SPARK_STROKE = {
    emerald: '#10b981', blue: '#3b82f6', purple: '#a855f7', orange: '#E8C65E'
};
...
export const StatCard = ({ label, value, sub, icon: Icon, theme, spark }) => {
    ...
    const themeStyles = {
        ...
        orange: "text-[#E8C65E] after:bg-[#E8C65E]", // Generic Gold/Orange
    };
...
export const ActionButton = ({ icon: Icon, label, onClick, color }) => {
    ...
    if (!isSpecial) {
        return (
            ...
            >
                ...
                <div className="relative p-3 mb-3 rounded-full bg-slate-50 dark:bg-slate-800 text-slate-400 group-hover:bg-[#E8C65E] group-hover:text-white transition-all duration-300 transform group-hover:scale-110 shadow-sm">
    ...
    return (
        <button
            onClick={onClick}
            className="w-full h-full relative group overflow-hidden rounded-2xl p-6 flex flex-row sm:flex-col items-center justify-between sm:justify-center gap-4 bg-[#0a0a0a] border border-[#E8C65E]/30 shadow-lg shadow-[#E8C65E]/10 hover:shadow-[#E8C65E]/20 transition-all duration-500 hover:-translate-y-1"
        >
            {/* Animated Background */}
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-[#E8C65E]/20 via-transparent to-transparent opacity-50" />

            <div className="relative p-3 rounded-xl bg-[#E8C65E]/10 text-[#E8C65E] border border-[#E8C65E]/20 group-hover:bg-[#E8C65E] group-hover:text-black transition-all duration-500">
                <Icon className="w-6 h-6 animate-pulse-slow" />
            </div>

            <div className="relative text-left sm:text-center">
                <span className="block text-[10px] text-[#E8C65E] font-mono mb-1 opacity-80">NEURAL CORE</span>
